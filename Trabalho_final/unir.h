...
// Paleta de cores ANSI utilizada para padronizar a TUI
...
// Interface visual básica reutilizada nas telas
void limparTela(void);
void pausarTela(void);
void limparEntrada(void);

// Exibe o logo do C-Bank e um título amigável
void cabecalho(const char *titulo); 

// Utilitários simples de texto usados com frequência
void apenasDigitos(const char *src, char *dst);
void trimNewline(char *s);
void removerQuebraLinha(char *str); // Alias para trimNewline (usado no cliente.c)

// Validações centralizadas para evitar repetição de código
int validarCPF(const char *cpf);
int senhaValidaNDigitos(const char *senha, int n);

// Leitura mascarada do CPF: somente primeiro e último visíveis
void lerCPFmascarado(char *out, size_t size);
// Leitura mascarada da senha: exibe asteriscos (Windows: alternância com TAB)
void lerSenhamascarada(char *out, size_t size);


// Wrappers de leitura para evitar erros com fgets/scanf
...
// Funções TUI em português utilizadas nas telas
...
// Barra de carregamento — wrapper para showLoading
...
// Geração de HTML (comprovantes/boletos/contratos) para enriquecer o projeto
// Gera um comprovante HTML de transferencia em `Trabalho_final/output/comprovante.html`
// Parâmetros: idOrigem, idDestino, valor e descrição opcional
...
// Buscas e geração de IDs centralizadas para reaproveitamento nos módulos
...
// Formata um CPF (somente dígitos) deixando início e fim visíveis
